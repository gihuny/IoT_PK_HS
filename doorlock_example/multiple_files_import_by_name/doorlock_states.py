...
import doorlock_gpios as GPIO_UTIL
...
#    check_events(SWITCH)
    ...
    while(True):
        # 번호확인
        if GPIO.event_detected(GPIO_UTIL.search_pin_switch(SWITCH, 'ENTER')) is True:
            break    # exit keypress
        elif GPIO.event_detected(GPIO_UTIL.search_pin_switch(SWITCH, '0')) is True:
            if num_of_keypress < 4:
                keypress_pattern[num_of_keypress] = 0
            num_of_keypress += 1
        elif GPIO.event_detected(GPIO_UTIL.search_pin_switch(SWITCH, '1')) is True:
            if num_of_keypress < 4:
                keypress_pattern[num_of_keypress] = 1
            num_of_keypress += 1
        elif GPIO.event_detected(GPIO_UTIL.search_pin_switch(SWITCH, '2')) is True:
            if num_of_keypress < 4:
